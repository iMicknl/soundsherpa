...
/// IEEE OUI (first three bytes of a MAC address) prefix tables used to guess the
/// vendor of a paired Bluetooth device when its name is not conclusive.
///
/// These are data, not logic: kept here so DeviceTypeResolver stays readable, and so
/// the tables can be extended without touching resolution code.
enum OUIPrefixes {
    /// Microsoft-registered OUI prefixes (uppercase, colon-separated).
    static let microsoft: Set<String> = [
    ...
    /// Apple-registered OUI prefixes (uppercase, colon-separated).
